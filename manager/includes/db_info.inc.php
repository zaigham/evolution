...
function output_db_info($dbase, $a, $backup_form)
    {
    global $modx, $_lang;
    ?>
	    ...
	    <form name="frmdb" method="post">
	        ...
	        <table class="db-list table">
		        ...
		        <tbody>
                    ...
                    $sql = 'SHOW TABLE STATUS FROM '.$dbase. ' LIKE \''.$modx->config['table_prefix'].'%\'';
                    ...
                    for ($i = 0; $i < $limit; $i++) {
	                    ...
	                    if($modx->hasPermission('settings') && in_array($db_status['Name'], $truncateable) && $db_status['Rows'] > 0) {
		                    echo "\t\t\t\t".'<td dir="ltr" class="table-data">'.
		                         '<a href="index.php?a=54&amp;mode='.$a.'&amp;u='.$db_status['Name'].'" title="'.$_lang['truncate_table'].'">'.nicesize($db_status['Data_length']+$db_status['Data_free']).'</a>'.
		                         '</td>'."\n";
	                    } else {
	                    ...
	                    if($modx->hasPermission('settings')) {
		                    echo "\t\t\t\t".'<td class="table-data">'.($db_status['Data_free'] > 0 ?
		                         '<a href="index.php?a=54&amp;mode='.$a.'&amp;t='.$db_status['Name'].'" title="'.$_lang['optimize_table'].'">'.nicesize($db_status['Data_free']).'</a>' :
		                         '-').
		                         '</td>'."\n";
	                    } else {
