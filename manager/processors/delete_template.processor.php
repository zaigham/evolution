...
if($limit>0) {
	$output .= "<b>This template is in use. Please set the documents using the template to another template. Documents using this template:</b><br />";
	for ($i=0;$i<$limit;$i++) {
		$row = $modx->db->getRow($rs);
		$output .= $row['id']." - ".$row['pagetitle']."<br />\n";
	}	
	$doDelete = FALSE;
}

if($id==$default_template) {
	$output .= "<b>This template is set as the default template. Please choose a different default template in the configuration before deleting this template.</b><br />";
	$doDelete = FALSE;
}
...
if($doDelete){
	...
	if(!$rs) {
		$output .= "Something went wrong while trying to delete the template...";
	} else {
